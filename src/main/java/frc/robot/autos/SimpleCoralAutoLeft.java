// Commands for driving forward starting at the left position,
// then stop, then drop coral into L1 in Auto Mode
...
public class SimpleCoralAutoLeft extends Command {
    private DriveSubsystem m_drive;
    private RollerSubsystem m_roller;
    private ArmSubsystem m_arm;
    private Timer timer;
    private double drive_seconds = 3.25;
    private double exjest_seconds = drive_seconds + 3;
     ...
     * @param arm
     */
    public SimpleCoralAutoLeft(DriveSubsystem drive, RollerSubsystem roller, ArmSubsystem arm)
    {
        m_drive = drive;
        m_roller = roller;
        m_arm = arm;
  ...
  public void execute() {
    ...
    //Drives for 1 seconds more to the left
    if(timer.get() > drive_seconds && timer.get() < drive_seconds + 1){

      m_drive.driveArcade(0, -0.3, false);

    }
    if(timer.get() > drive_seconds + 1 && timer.get() < drive_seconds + 2){
      // Drive fwd at 30% for 1 sec
      m_drive.driveArcade(0.3, 0, false);
     ...
     */
    else if(timer.get() > drive_seconds + 2 && timer.get() < exjest_seconds)
    {
