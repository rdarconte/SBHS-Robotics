// Commands for driving forward, then stop, then drop coral into L1 in Auto Mode
...
public class SimpleCoralAuto extends Command {
    ...
    private double exjest_seconds = drive_seconds + 1.8;
  ...
  public void execute() {
    ...
    /**
     * While the timer is less than drive_seconds, the robot will obey the command inside
     */
    ...
    else if(timer.get() > drive_seconds && timer.get() < drive_seconds + 0.6){
      // stop, and run arm at 20%
     ...
     */
    else if(timer.get() > drive_seconds + 0.6 && timer.get() < exjest_seconds)
    {
        m_drive.driveArcade(0.0, 0.0,false);
